...
export const Story = ({title = 'Deine Geschichte', filterText = () => false, filterBild = () => false}) => {
    ...
    const handleBackgroundChange = (value) => {
        if (!story) return;
        const bg = value ? {bildId: value.bildId, opacity: value.opacity, tint: value.tint, offsetX: value.offsetX ?? 0, offsetY: value.offsetY ?? 0, zoom: value.zoom || 1, outpaintedPfad: value.outpaintedPfad ?? null, caption: value.caption ?? null} : null;
