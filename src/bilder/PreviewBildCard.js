...
export const PreviewBildCard = memo(({bild, story, storiesLoaded, stories, onSetComplete, storyBilder = [], storyTexte = []}) => {
    const [updateBild] = api.endpoints.updateBild.useMutation();
    ...
    return (
        <Paper elevation={1} sx={{p: 2, display: 'flex', flexDirection: 'column', position: 'relative',
            ...
            <Box sx={{display: 'flex', flexDirection: 'column', flex: 1, pt: 3}}>
                ...
                <Box sx={{mb: 2}}>
                    <AuthImage src={bild.pfad?.startsWith('/') ? `/api/bilder/extern${bild.pfad}` : bild.pfad}
                        alt={bild.description || ''} thumb style={{width: '100%', height: 'auto', display: 'block'}}/>
                </Box>
                ...
                actionButtons={
                    <>
                        ...
                        <Tooltip title={bild.hauptbild ? "Hauptbild – klicken zum Entfernen" : "Als Hauptbild markieren"}>
                            <IconButton size="small" onClick={e => { e.stopPropagation(); setHauptbild({bild, hauptbild: !bild.hauptbild}); }}
